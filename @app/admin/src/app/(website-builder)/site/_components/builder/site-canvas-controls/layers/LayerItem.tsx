...
const LayerItem: FC<LayerItemProps> = ({
  ...
  return (
    <div
      ...
      className={twMerge(
        'group relative flex h-[38px] items-center justify-between border border-transparent px-2 text-xs hover:border-violet-900',
        className,
        selectedComonentPath === id && 'border-violet-900 bg-violet-200/50'
      )}
    >
      <div data-layer-item className="z-10 flex items-center gap-2">
        <div>{startContent}</div>
        <div data-layer-item className="h-full w-full flex-1">
          <div
            data-layer-item
      ...
      <div
        className={twMerge(
          'hidden space-x-1 group-hover:block',
